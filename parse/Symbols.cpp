...
//        retVal = build.CreateLoad(getAddress(), getName());
//	}
//	return retVal;
...
SymbolTable::SymbolTable() noexcept
{
    ...
    Identifier* dFuntion = createIdentifier("@@function");
    dFuntion->mType = Type::Function;
    Identifier* dvar = createIdentifier("@@variable");
    dvar->mType = Type::Int;
    Identifier* dprint = createIdentifier("printf");
    dprint->setType(Type::Function);
    mCurrScope->addIdentifier(dFuntion);
    mCurrScope->addIdentifier(dprint);
    mCurrScope->addIdentifier(dvar);
}
...
{
	// PA2: Implement
    ScopeTable* childScope = new ScopeTable(mCurrScope);
    mCurrScope = childScope;
	return mCurrScope;
}
...
{
	// PA2: Implement
    if(mSymbols.find(ident->getName()) == mSymbols.end()){
...
Identifier* SymbolTable::ScopeTable::search(const char* name) noexcept
{
    ...
    while(scope != nullptr && retVal == nullptr){
        retVal = scope->searchInScope(name);
        scope = scope->mParent;
    }
...
//            decl = build.CreateAlloca(ident->llvmType());
//            if(ident->getAddress()){
