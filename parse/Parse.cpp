...
Identifier* Parser::getVariable(const char* name) noexcept
{
    ...
    auto var = mSymbols.getIdentifier(name);
    if(var == nullptr){
        std::string msg = "Use of undeclared identifier '";
        msg += name;
        msg += "'";
        reportSemantError(msg);
        var = mSymbols.getIdentifier("@@variable");
    }
    return var;
}
...
std::shared_ptr<ASTExpr> Parser::charToInt(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	// PA2: Implement
    if(expr != nullptr && expr->getType() == Type::Char){
        auto constNode = std::dynamic_pointer_cast<ASTConstantExpr>(expr);
        if(nullptr != constNode){
            constNode->changeToInt();
        }
        else{
            retVal = make_shared<ASTToIntExpr>(expr);
        }
    }
	return retVal;
}
...
std::shared_ptr<ASTExpr> Parser::intToChar(std::shared_ptr<ASTExpr> expr) noexcept
{
	...
	// PA2: Implement
    if(expr != nullptr && expr->getType() == Type::Int){
        auto toIntNode = std::dynamic_pointer_cast<ASTToIntExpr>(expr);
        if(nullptr != toIntNode){
            retVal = toIntNode->getChild();
        }
        else{
            auto constNode = std::dynamic_pointer_cast<ASTConstantExpr>(expr);
            if(nullptr != constNode){
                constNode->changeToChar();
            }
            else{
                retVal = make_shared<ASTToCharExpr>(expr);
            }
        }
    }
