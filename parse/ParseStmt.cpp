...
shared_ptr<ASTCompoundStmt> Parser::parseCompoundStmt(bool isFuncBody)
{
    ...
    if(peekAndConsume(Token::LBrace)){
        ...
        SymbolTable::ScopeTable* scope = nullptr;
        if(!isFuncBody){
            scope = mSymbols.enterScope();
        }
        ...
        shared_ptr<ASTStmt> stat;
        while((stat = parseStmt())){
            retVal->addStmt(stat);
        }
        
        //Match the last token to "}"
        
        if(isFuncBody){
            auto lastStmt = std::dynamic_pointer_cast<ASTReturnStmt>(retVal->getLastStmt());
            if(lastStmt == nullptr){
	...
	if (peekToken() == Token::Identifier)
	{
		...
		if (peekAndConsume(Token::Assign))
		{
			...
			else
			{
                ...
                if(ident->getType() == Type::IntArray ||
...
shared_ptr<ASTIfStmt> Parser::parseIfStmt()
{
    ...
    if(peekAndConsume(Token::Key_if)){
        ...
        //Parse Condtion
        shared_ptr<ASTExpr> cond = parseExpr();
        if(!cond){
        ...
        //Parse body
        shared_ptr<ASTStmt> body = parseStmt();
        ...
        //Construct ifStmt
        retVal = make_shared<ASTIfStmt>(cond, body, elsestmt);
...
shared_ptr<ASTWhileStmt> Parser::parseWhileStmt()
{
    ...
    if(peekAndConsume(Token::Key_while)){
        ...
        shared_ptr<ASTStmt> body = parseStmt();
        if(!body){
            body = make_shared<ASTNullStmt>();
        }
        //Construct whilestmt
        retVal = make_shared<ASTWhileStmt>(cond, body);
...
shared_ptr<ASTReturnStmt> Parser::parseReturnStmt()
{
    ...
    if(peekAndConsume(Token::Key_return)){
        //Parse expr after return
        unsigned int col = mColNumber;
        shared_ptr<ASTExpr> expr = parseExpr();
        if(expr == nullptr && mCurrReturnType != Type::Void){
