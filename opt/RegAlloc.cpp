...
public:
    ...
    , present(new bool[n])
    {
        ...
        for(int i = 0; i < n; ++i){
            matrix[i] = new bool[n];
            memset(matrix[i], 0, n * sizeof(bool));
            present[i] = true;
        }
    }
...
void RAUSCC::initGraph() {
    ...
    for(unsigned int i = 0; i != MRI->getNumVirtRegs(); ++i){
        ...
        if(MRI->reg_nodbg_empty(Reg))
            continue;
        intervals.emplace_back(&LIS->getInterval(Reg));
    }
    std::cout << intervals.size() << std::endl;
    mGraph = new Graph(intervals.size());
...
void RAUSCC::simplifyGraph() {
    ...
    while(!mGraph->Empty()){
        bool trivial = false;
        for(int i = 0; i < intervals.size(); ++i){
            if(mGraph->NodePresent(i)){
                int deg = mGraph->Degree(i);
                if(deg < NUM_COLORS){
                    printf("Found neighbors=%d for ", deg);
                    intervals[i]->dump();
                    printf("Removal %d: ", rm);
                    intervals[i]->dump();
                    mGraph->RemoveNode(i);
        ...
        if (!trivial) {
            ...
            for(int j = 0; j < intervals.size(); ++j){
                if(mGraph->NodePresent(j) && (!spill || intervals[j]->weight < spill->weight)){
                    spill = intervals[j];
                    idx = j;
                }
            }
            ...
            std::cout.flush();
            printf("Spill candidate (neighbors=%d, weight= %f): ", mGraph->Degree(idx), spill->weight);
            spill->dump();
            printf("Removal %d: ", rm);
            spill->dump();
            
            mGraph->RemoveNode(idx);
            ++rm;
            ++gen;
        }
    }
